import {
 ...
 async function handleRedirectAfterLogin() {
   ...
   if (session.info.isLoggedIn) {
     ...
     document.getElementById(
       "labelStatus"
     ).innerHTML = `Your session is logged in with the WebID [<a target="_blank" href="${session.info.webId}">${session.info.webId}</a>]. To upload files to your SolidPod server: [<a target="_blank" href="https://konvolut.solidcommunity.net/">https://konvolut.solidcommunity.net/</a>].`;
...
// 2.a. Write to profile
async function writeFileProfile() {
  const name = document.getElementById("myFile").value;
  ...
  document.getElementById(
    "labelWriteStatusFile"
  ).textContent = `Wrote [${name}] at ${getSourceUrl(savedFile)} successfully!`;
  document.getElementById("labelWriteStatusFile").setAttribute("role", "alert");
  } catch (error) {
    console.error(error);
    document.getElementById(
      "labelWriteStatusFile"
    ).textContent = `Failed to write [${name}] at ${getSourceUrl(savedFile)}!`;
    document.getElementById("labelWriteStatusFile").setAttribute("role", "alert");
  }
